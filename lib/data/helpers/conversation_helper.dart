...
class ConversationHelper {
  ...
  Future<Chat?> initChatIfExists() async {
    ...
    List<Map<String, dynamic>> chatsId = await _chatUsersTable.getChatsByUserId(user!.uid);
  ...
  Future<Chat?> createNewChat() async {
    ...
    String id = (await _firestore.collection(FirestoreCollection.chats).add({'users': [user!.uid, UserManager.uid], 'type': chat?.type.name})).id;
    chat!.setId(id);
