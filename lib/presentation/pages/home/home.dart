...
import 'package:whatsapp/presentation/pages/home/community/community_page.dart';
...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  final ValueNotifier<int> _unreadChatsCountNotifier = ValueNotifier(0);
  final ValueNotifier<bool> _newStatusUpdateNotifier = ValueNotifier(false);
  ...
  void initState() {
    ...
    _tabController =
        TabController(initialIndex: _initialTabIndex, length: 4, vsync: this);
    _searchViewHeightChangeNotifier =
        ValueNotifier(_searchViewHeight[_initialTabIndex]);
    _selectCountProvider =
        Provider.of<SelectCountProvider>(context, listen: false);
    ...
    _tabController.animation?.addListener(() {
      debugPrint("Animating... ${_tabController.animation?.value}");
      if (_tabController.indexIsChanging) {
        // _tabChangeNotifier.value = _tabController.index;
        debugPrint(
            "Animating index changing... ${_tabController.animation?.value}");
      }
    });
  ...
  void _initUser() async {
    if(UserManager.whatsAppUser == null) {
      await UserManager.initWhatsAppUser();
    }
  }
          ...
          builder: (context, value, child) {
            return Visibility(
              ...
              child: FloatingActionButton(
                onPressed: () => _onFabPressed(value),
                foregroundColor: themeMode == ThemeMode.dark ? Colors.black : Colors.white,
                child: _buildFabChild(value),
              ),
            );
          },
        ),
        ...
        builder: (context, child) => ValueListenableBuilder(
          ...
          builder: (context, inSelectMode, child) {
            ...
            SystemChrome.setSystemUIOverlayStyle(
                SystemUiOverlayStyle(statusBarColor: surfaceColor));
            ...
            return SliverAppBar(
              ...
              floating: true,
              snap: true,
              toolbarHeight: value <= 56
                  ? 56 * (1 - _searchAnimationController.value)
                  : 56,
              expandedHeight: value > 56
                  ? (value - 56) * _searchAnimationController.value
                  : null,
              elevation: 0,
              title: !inSelectMode
                  ? const Text("WhatsApp")
  ...
  Widget _buildBody() {
    return TabBarView(controller: _tabController, children: [
      ...
      ChatList(
        ...
        unreadChatsCountNotifier: _unreadChatsCountNotifier,
      ),
      const StatusPage(),
      const Center(
        child: Text("Calls"),
  ...
  Widget _buildFabChild(int tabIndex) {
    return switch (tabIndex) {
      1 => const Icon(WhatsAppIcons.message),
      2 => const Icon(WhatsAppIcons.camera_fill),
      3 => const Icon(Icons.add_call),
      _ => const SizedBox(),
    };
  }
  ...
  void _onFabPressed(int index) {
    switch (index) {
      case 1:
        _chatListKey.currentState?.gotoDeviceContacts();
        break;
      case 2:
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, inSelectMode, child) => SliverPersistentHeader(
          ...
          delegate: TabBarHeader(
            ...
            unreadChatsCountNotifier: _unreadChatsCountNotifier,
            newStatusUpdateNotifier: _newStatusUpdateNotifier,
          )),
    );
  }
            ...
            return CircularRevealAnimation(
              ...
              child: ValueListenableBuilder(
                ...
                builder: (context, value, child) => WhatsAppSearch(
                  height: value,
                  showOptionChips: _tabController.index == 1,
  ...
  void _afterWidgetsLoad(Duration duration) {
    ...
    if (!_isOffsetLoaded) {
      RenderBox? actionSearchRenderBox =
          _actionSearchKey.currentContext?.findRenderObject() as RenderBox?;
      ...
      if (actionSearchRenderBox != null) {
        ...
        _actionSearchStreamController.add(
            actionSearchRenderBox.localToGlobal(Offset.zero) +
                Offset(buttonSize.width / 2, -buttonSize.height / 4));
      }
    }
  }
  ...
  void _tabChangeListener() {
    ...
    _searchViewHeightChangeNotifier.value =
        _searchViewHeight[_tabController.index];
    _tabChangeNotifier.value = _tabController.index;
  }
