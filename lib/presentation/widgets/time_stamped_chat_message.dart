...
class TimestampedChatMessage extends LeafRenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    TextStyle? effectiveMessageTextStyle = messageStyle;
    TextStyle? effectiveTimeTextStyle = timeStyle;
    if (messageStyle == null || messageStyle!.inherit) {
      effectiveMessageTextStyle = defaultTextStyle.style.merge(messageStyle);
    }
    if (timeStyle == null || timeStyle!.inherit) {
      effectiveTimeTextStyle = defaultTextStyle.style.merge(timeStyle);
    }
    return TimestampedChatMessageRenderObject(
      text: text,
      sentAt: sentAt,
      textDirection: Directionality.of(context),
      textStyle: effectiveMessageTextStyle!,
      sentAtStyle: effectiveTimeTextStyle!,
      statusIconSize: stateIconSize,
    );
  }
  ...
  void updateRenderObject(
    ...
    renderObject.textStyle = effectiveMessageTextStyle!;

    renderObject.sentAt = sentAt;
    renderObject.sentAtStyle = effectiveTimeTextStyle!;
...
class TimestampedChatMessageRenderObject extends RenderBox {
  ...
  Size _layoutText(double maxWidth) {
    ...
    _sentAtLineWidth = _sentAtTextPainter.computeLineMetrics().first.width + _statusIconSize;
    _sentAtLineHeight = _sentAtTextPainter.computeLineMetrics().first.height;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_sentAtFitsOnLastLine) {
      sentAtOffset = Offset(
        ...
        offset.dy + (_lineHeight * (_numMessageLines - 1)) + (_lineHeight - _sentAtLineHeight),
      );
    } else {
      sentAtOffset = Offset(
        offset.dx + (size.width - _sentAtLineWidth),
        offset.dy + (_lineHeight * _numMessageLines) + (_lineHeight - _sentAtLineHeight),
      );
    }
